...
int _treCBFParentRetriver(void * pitem, size_t param)
{
	...
	if (*(P_TNODE_B *)param == ((P_TNODE_B)pitem)->ppnode[LEFT])
	{
		*(P_TNODE_B *)param = (P_TNODE_B)pitem;
		return CBF_TERMINATE;
	}
	if (*(P_TNODE_B *)param == ((P_TNODE_B)pitem)->ppnode[RIGHT])
	{
		*(P_TNODE_B *)param = (P_TNODE_B)pitem;
		return CBF_TERMINATE;
	}
...
int _treCBFCopyTreeNodeB(void * pitem, size_t param)
{
	...
	P_TNODE_B pcur = (P_TNODE_B)pitem;
	P_TNODE_B pnew = strCreateNodeD(pcur->pdata, ptc->size);
	if (NULL == pnew)
	{	/* Allocation failure. */
		treFreeB((P_BTREE)&ptc->pnroot);
		ptc->pnroot = NULL;
		return CBF_TERMINATE;
	}
	/* Set new root. */
	...
	if (NULL != pcur->ppnode[LEFT])
	{
		...
		queInsertL(ptc->pquel, &tptr, sizeof(P_TNODE_B));
	}
...
int treTraverseBPre(P_TNODE_B pnode, CBF_TRAVERSE cbftvs, size_t param)
{
	...
	if (CBF_CONTINUE != cbftvs(pnode, param))
		return CBF_TERMINATE;
	r1 = treTraverseBPre(pnode->ppnode[LEFT],  cbftvs, param);
	r2 = treTraverseBPre(pnode->ppnode[RIGHT], cbftvs, param);
	return CBF_CONTINUE == r1 ? r2 : r1;
}

/* Function name: treTraverseBIn
...
int treTraverseBIn(P_TNODE_B pnode, CBF_TRAVERSE cbftvs, size_t param)
{
	...
	if (NULL == pnode)
		return CBF_CONTINUE;
	r1 = treTraverseBIn(pnode->ppnode[LEFT],  cbftvs, param);
	if (CBF_CONTINUE != cbftvs(pnode, param))
		return CBF_TERMINATE;
	r2 = treTraverseBIn(pnode->ppnode[RIGHT], cbftvs, param);
	return CBF_CONTINUE == r1 ? r2 : r1;
}

/* Function name: treTraverseBPost
...
int treTraverseBPost(P_TNODE_B pnode, CBF_TRAVERSE cbftvs, size_t param)
{
	...
	if (NULL == pnode)
		return CBF_CONTINUE;
	r1 = treTraverseBPost(pnode->ppnode[LEFT],  cbftvs, param);
	r2 = treTraverseBPost(pnode->ppnode[RIGHT], cbftvs, param);
	if (CBF_CONTINUE != cbftvs(pnode, param))
		return CBF_TERMINATE;
	return CBF_CONTINUE == r1 ? r2 : r1;
}

/* Function name: treTraverseBLevel
...
int treTraverseBLevel(P_TNODE_B pnode, CBF_TRAVERSE cbftvs, size_t param)
{
	...
	QUEUE_L q;
	queInitL(&q);
	queInsertL(&q, &pnode, sizeof(P_TNODE_B));
	while (! queIsEmptyL(&q))
	{
		queRemoveL(&pnode, sizeof(P_TNODE_B), &q);
		if (CBF_CONTINUE != cbftvs(pnode, param))
			return CBF_TERMINATE;
		if (NULL != pnode->ppnode[LEFT])
			r1 = queInsertL(&q, &(pnode->ppnode[LEFT]),  sizeof(P_TNODE_B));
		if (NULL != pnode->ppnode[RIGHT])
			r2 = queInsertL(&q, &(pnode->ppnode[RIGHT]), sizeof(P_TNODE_B));
	}
...
/* Function name: treFreeB
 * Description:   Deallocate a binary tree of which is allocated by function treInitB_O.
 * Parameter:
...
void treFreeB(P_BTREE ptreb)
{
	 ...
	 */
	treTraverseBPost(*ptreb, _strCBFDeleteNode, ENT_DOUBLE);
	*ptreb = NULL;
}

/* Function name: treCreateB
...
{
	P_BTREE ptreb = (P_BTREE) malloc(sizeof(BTREE));
	if (NULL == ptreb)
		return NULL;
	treInitB(ptreb);
	return ptreb;
...
/* Function name: treDeleteB_O
 * Description:   Delete a binary tree of which is allocated by function treCreateB.
 * Parameter:
...
{
	treTraverseBPost(pnode->ppnode[LEFT], _strCBFDeleteNode, ENT_DOUBLE);
	pnode->ppnode[LEFT] = NULL;
}

/* Function name: treRemoveRightB
...
{
	treTraverseBPost(pnode->ppnode[RIGHT], _strCBFDeleteNode, ENT_DOUBLE);
	pnode->ppnode[RIGHT] = NULL;
}

/* Function name: treArityB
...
size_t treArityB(P_TNODE_B pnode)
{
	...
	treTraverseBIn(pnode, _strCBFNodesCounter, (size_t)&s);
	return s;
}

/* Function name: treHeightB
...
size_t treHeightB(P_TNODE_B pnode)
{
	...
	if (NULL == pnode)
		return 0;
	dl = treHeightB(pnode->ppnode[LEFT]);
	dr = treHeightB(pnode->ppnode[RIGHT]);
	if (dl > dr)
		return dl + 1;
	else
		return dr + 1;
...
/* Function name: treGetParentNodeB
...
P_TNODE_B treGetParentNodeB(P_TNODE_B proot, P_TNODE_B pchild)
{
	...
	treTraverseBPre(proot, _treCBFParentRetriver, (size_t)&pchild);
	return pchild;
}

/* Function name: treSearchDataB
...
P_TNODE_B treSearchDataB(P_TNODE_B pnode, const void * pitem, size_t size, TvsMtd tm)
{
	...
	switch (tm)
	{
	case ETM_PREORDER:   treTraverseBPre  (pnode, _strCBFCompareNodeDataD, (size_t)&fi); break;
	case ETM_INORDER:    treTraverseBIn   (pnode, _strCBFCompareNodeDataD, (size_t)&fi); break;
	case ETM_POSTORDER:  treTraverseBPost (pnode, _strCBFCompareNodeDataD, (size_t)&fi); break;
	case ETM_LEVELORDER: treTraverseBLevel(pnode, _strCBFCompareNodeDataD, (size_t)&fi); break;
	}
	return (P_TNODE_B)fi.result;
}

/* Function name: treDescendantB
...
P_TNODE_B treMergeNodesB(P_TNODE_B proot, const void * pitem, size_t size, P_TNODE_B pleft, P_TNODE_B pright)
{
	...
	if (treDescendantB(pleft, pright) || treDescendantB(pright, pleft))
		return NULL;
	if (NULL == proot)
...
P_TNODE_B treSwapNodesB(P_TNODE_B proot1, P_TNODE_B pnode1, P_TNODE_B proot2, P_TNODE_B pnode2)
{
	if (NULL == proot1 || NULL == proot2 ||
		...
		treDescendantB(pnode1, pnode2)   ||
		treDescendantB(pnode2, pnode1)
		) return NULL; /* T1 contains T2, or T2 contains T1. */
	else
	{
		P_TNODE_B prt1, prt2;
		prt1 = treGetParentNodeB(proot1, pnode1);
		prt2 = treGetParentNodeB(proot2, pnode2);
...
P_TNODE_B treCopyB(P_TNODE_B proot, size_t size)
{
	...
	tp.pquel = &q;
	tp.size = size;
	treTraverseBLevel(proot, _treCBFCopyTreeNodeB, (size_t)&tp);
	 ...
	 * exiting function treCopyB to prevent a potential memory leak.
	 */
	queFreeL(&q);
	return (P_TNODE_B)tp.pnroot;
}
